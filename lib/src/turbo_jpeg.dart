...
class TurboJpeg {
  ...
  Uint8List decompress(
    ...
    _handle = _bindings.tjInitDecompress();
    if (_handle == ffi.nullptr) {
      throw Exception('Failed to initialize TurboJPEG decompressor instance');
    }
    _isInitialized = true;
    _finalizer.attach(this, _handle);

    final int dstSize = width * height * 4;

    return using((Arena arena) {
      final ffi.Pointer<ffi.Uint8> jpegBuf = arena<ffi.Uint8>(jpegData.length);
      final Uint8List jpegList = jpegBuf.asTypedList(jpegData.length);
      jpegList.setAll(0, jpegData);

      final ffi.Pointer<ffi.Uint8> dstBuf = arena<ffi.Uint8>(dstSize);

      final int result = _bindings.tjDecompress2(
        _handle,
        jpegBuf.cast<ffi.UnsignedChar>(),
        jpegData.length,
        dstBuf.cast<ffi.UnsignedChar>(),
        width,
        0,
        height,
        pixelFormat,
      ...
      if (result != 0) {
        final ffi.Pointer<ffi.Char> err = _bindings.tjGetErrorStr2(_handle);
        final String errorStr = err.cast<Utf8>().toDartString();
        throw Exception('TurboJPEG decompression failed: $errorStr');
      }

      final Uint8List dartResult =
          Uint8List.fromList(dstBuf.asTypedList(dstSize));
